...
<script type="text/javascript">
  ...
  function updatePlaylist(object) {
    ...
    var attachment_id = object.id.replace("attachments[", "").replace("][enabled]", "");
    if (object.checked === true) {
      p_items.push(attachment_id);
    } else {
      p_items.splice(attachment_id, 1);
    }
    alert(p_items);
...
function get_playlist_item($attachment_id, $args = null, $current_playlist) {
  ...
  foreach ($playlist_items as $playlist_item) {
	...
	foreach ( $form_fields as $key => $val ) {
        ...
        $order .= '<input class="menu_order_input" type="checkbox" id="attachments['.$attachment_id.'][enabled]" name="attachments['.$attachment_id.'][enabled]" value="'.$val['value'].'"'.$checked.' onchange="updatePlaylist(this);" /></div>';
      } else
...
function get_jw_playlist_item($attachment_id, $args = null, $current_playlist) {
	global $redir_tab;
  ...
  foreach ($playlist_items as $playlist_item) {
	...
	$display_title
	<table class='slidetoggle describe $class'>
		<thead class='playlist-item-info' id='playlist-head-$post->ID'>
		<tr>
			<td class='A1B1' id='thumbnail-head-$post->ID' rowspan='5'><img class='thumbnail' src='$thumb_url' alt='' /></td>
			<td><strong>" . __('File name:') . "</strong> $filename</td>
		</tr>
