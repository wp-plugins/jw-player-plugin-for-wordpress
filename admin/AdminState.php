          ...
          function saveHandler(button) {
            ...
            for (var config in configs) {
              if (newVal.value == configs[config]) {
                return confirm("<?php _e("A player with this name already exists and will be overwritten.  Would you like to continue?"); ?>");
              }
            }
            return true;
          }
        </script>
        <input align="left" class="button-primary" type="submit" name="Save" value="<?php _e("Save"); ?>" onclick="return saveHandler(this);"/>
      <?php } ?>
      <?php if ($show_previous) { ?><input type="submit" name="Previous" value="<?php _e("Previous"); ?>" /><?php } ?>
      <?php if ($show_cancel) { ?><input align="right" type="submit" name="Cancel" value="<?php _e("Cancel"); ?>" /><?php } ?>
      <?php if ($show_next) { ?><input align="right" type="submit" name="Next" value="<?php _e("Next"); ?>" /><?php } ?>
    </p><?php
  }
